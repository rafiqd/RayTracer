...
Hitable* randomScene(){
    ...
    for(int a = -10; a < 10; a++){
        for (int b = -10; b < 10; b++){
            float chooseMat = Distributions::zero_to_one.getRandFloat();
            Vector3f center(a+0.9f*Distributions::zero_to_one.getRandFloat(),
                            0.2,
                            b+0.9f*Distributions::zero_to_one.getRandFloat());
            if((center - Vector3f(4,0.2, 0)).length() > 0.9){
                if (chooseMat < 0.8){
                    list[i++] = new MovingSphere(center,
                                                 center+Vector3f(0, 0.5f*Distributions::zero_to_one.getRandFloat(), 0),
                                                 0, 1,
                                                 0.2,
                                                 new Lambertian(new ConstantTexture(
                                                         Vector3f(Distributions::zero_to_one.getRandFloat()*Distributions::zero_to_one.getRandFloat(),
                                                                  Distributions::zero_to_one.getRandFloat()*Distributions::zero_to_one.getRandFloat(),
                                                                  Distributions::zero_to_one.getRandFloat()*Distributions::zero_to_one.getRandFloat()))
                                                 )
                    );
                    ...
                    list[i++] = new Sphere(center,
                                           0.2,
                                           new Metal(Vector3f(0.5f*(1 + Distributions::zero_to_one.getRandFloat()),
                                                              0.5f*(1 + Distributions::zero_to_one.getRandFloat()),
                                                              0.5f*(1 + Distributions::zero_to_one.getRandFloat())),
                                           0.5f*Distributions::zero_to_one.getRandFloat()));
